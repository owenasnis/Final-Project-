...
white_12 <- stan_glm(data = countypres12, 
         formula = rep_vs ~ white_pct, 
         refresh = 0) %>% 
  as_tibble() %>% 
  select(white_pct) %>% 
  mutate(year = "2012")

white_16 <- stan_glm(data = countypres16, 
         formula = rep_vs ~ white_pct, 
         refresh = 0) %>% 
  as_tibble() %>% 
  select(white_pct) %>% 
...
white_predictions <- bind_rows(white_12, white_16)

ggplot(data = white_predictions, aes(x = white_pct, fill = year)) + 
