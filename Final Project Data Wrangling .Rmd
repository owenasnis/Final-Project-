...
plot_usmap(include = c("WI", "MI", "PA"), 
    ...
    scale_fill_gradient2(name = "Swing", 
                         ...
                         labels = c("+10% D", "Even", "+10% R", 
                                    "+20% R", "+30% R")) + 
  labs(title = "Swings from 2012 to 2016", 
       subtitle = "Republicans gain in rural/suburban areas")
...
full_join(top_25_summary_stats, summary_stats, 
          by = c("white_pct", "nonwhite_pct", "less_college_pct", 
                 "college_more_pct", "avg_pop")) %>% 
  mutate(from = c("top 25 swings", "all")) 

```

```{r models}

countypres2012 <- countypres %>% 
  filter(year == 2012) %>% 
  ungroup()

countypres2016 <- countypres %>% 
  filter(year == 2016) %>% 
  ungroup()
